...
export async function selectUser(req, res) {
    ...
    try {
        await openDb().then(db => {
            db.get('SELECT * FROM user WHERE email=?', [user.email])
...
export async function deleteUser(req, res) { //documentado
    let user = req.body
    let db = await openDb()
    ...
    try {
        if (!user.email) {
            ...
            await db.get('DELETE FROM user WHERE email=?', [user.email])
            res.status(200).json({
...
export async function authUser(req, res) { //documentado
    let user = req.body;

    try {
        const db = await openDb();
        const userResponse = await db.get('SELECT * FROM user WHERE email=?', [user.email]);

        if (userResponse && user.email === userResponse.email && user.password === userResponse.password) {
            const token = jwt.sign({ userId: user.email }, SECRET, { expiresIn: 300 });
